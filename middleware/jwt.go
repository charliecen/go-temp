...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err, _ = userService.FindUserByUuid(claims.UUID.String()); err != nil {
			_ = jwtService.JsonInBlacklist(user.JwtBlacklist{Jwt: token})
			response.FailWithDetailed(gin.H{"reload": true}, err.Error(), c)
			c.Abort()
		}
		if claims.ExpiresAt-time.Now().Unix() < claims.BufferTime {
			...
			/*if global.CONFIG.user.UseMultipoint {
				...
				if err != nil {
					...
					_ = jwtService.JsonInBlacklist(user.JwtBlacklist{Jwt: RedisJwtToken})
				}
				// 无论如何都要记录当前的活跃状态
...
func (j *JWT) CreateTokenByOldToken(oldToken string, claims request.CustomClaims) (string, error) {
	v, err, _ := global.CC.Do("JWT:"+oldToken, func() (interface{}, error) {
